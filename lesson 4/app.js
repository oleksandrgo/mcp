(function () {
  ...
  function getDiagramRoot(index) {
    ...
    return slide.querySelector(".mcp-flow, .mcp-seq-flow");
  }

  function getDiagramSteps(index) {
    const slide = slides[index];
    if (!slide) return [];
    return slide.querySelectorAll(".mcp-flow__step, .mcp-seq-flow__step");
  }

  function getSeqFlowBottomParticipants(slide) {
    return slide?.querySelector(".mcp-seq-flow__participants--bottom") ?? null;
  }

  function isSeqFlowTopParticipants(el) {
    return (
      el?.classList.contains("mcp-seq-flow__participants") &&
      !el.classList.contains("mcp-seq-flow__participants--bottom")
    );
  }
  ...
  function resetDiagramSteps(index) {
    ...
    const bottom = getSeqFlowBottomParticipants(slides[index]);
    if (bottom) gsap.set(bottom, { opacity: 0, y: 16, scale: 0.96 });
    diagramRevealCounts.set(index, 0);
    const root = getDiagramRoot(index);
    root?.classList.remove("mcp-flow--streaming", "mcp-seq-flow--streaming");
    slides[index]?.querySelectorAll(".mcp-seq-flow__row--live").forEach((row) => {
      row.classList.remove("mcp-seq-flow__row--live");
    });
  }

  function revealDiagramElements(elements, onComplete) {
    gsap.fromTo(
      elements,
  ...
  function hideDiagramElements(elements, onComplete) {
    gsap.to(elements, {
      opacity: 0,
      y: -12,
      scale: 0.96,
  ...
  function revealNextDiagramStep() {
    ...
    const root = getDiagramRoot(current);
    const bottom = isSeqFlowTopParticipants(el)
      ? getSeqFlowBottomParticipants(slides[current])
      : null;
    const targets = bottom ? [el, bottom] : el;

    diagramRevealCounts.set(current, shown + 1);
    revealDiagramElements(targets, () => {
      if (el.classList.contains("mcp-seq-flow__row")) {
        el.classList.add("mcp-seq-flow__row--live");
      } else if (shown + 1 >= steps.length) {
        root?.classList.add("mcp-flow--streaming");
      }
    });
    return true;
  }
  ...
  function hideLastDiagramStep() {
    ...
    root?.classList.remove("mcp-flow--streaming", "mcp-seq-flow--streaming");
    if (el.classList.contains("mcp-seq-flow__row")) {
      el.classList.remove("mcp-seq-flow__row--live");
    }
    hideDiagramElements(targets);
    return true;
  }
